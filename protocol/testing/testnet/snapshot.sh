...
SNAP_PATH="/nemo-network/chain/.full-node-0/snapshots/nemo-network/"
# logfile containing snapshot timestamps
LOG_PATH="/nemo-network/chain/.full-node-0/snapshots/nemo-network/nemo-network_log.txt"
# data directory to snapshot. this contains the blockchain state.
DATA_PATH="/nemo-network/chain/.full-node-0/data/"
...
sed -i 's/pruning = "default"/pruning = "everything"/' /nemo-network/chain/.full-node-0/config/app.toml
...
sed -i 's/min-retain-blocks = 0/min-retain-blocks = 2/' /nemo-network/chain/.full-node-0/config/app.toml
# Do not index tx_index.db
sed -i 's/indexer = "kv"/indexer = "null"/' /nemo-network/chain/.full-node-0/config/config.toml
...
while true; do
  ...
  cosmovisor run start --log_level info --home /nemo-network/chain/.full-node-0 --p2p.seeds "${p2p_seeds}" --non-validating-full-node=true --dd-agent-host=${dd_agent_host} &
