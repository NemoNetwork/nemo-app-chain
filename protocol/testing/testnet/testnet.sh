...
set_cosmovisor_binary_permissions() {
    ...
    for version in "${!version_to_url[@]}"; do
        ...
        mv "$version_dir/build/$binary_file" "$version_dir/bin/nemo-networkd"
        chmod 755 "$version_dir/bin/nemo-networkd"
        echo "Successfully set up $version_dir/bin/nemo-networkd"
    done
    current_version_path="$HOME/cosmovisor/upgrades/$CURRENT_VERSION/bin"
    mkdir -p $current_version_path
    cp /bin/nemo-networkd $current_version_path
}
...
create_validators() {
	...
	for i in $(seq 0 $LAST_FULL_NODE_INDEX); do
		...
		nemo-networkd init "full-node" -o --chain-id=$CHAIN_ID --home "$FULL_NODE_HOME_DIR"
	...
	for i in "${!MONIKERS[@]}"; do
		...
		nemo-networkd init "${MONIKERS[$i]}" -o --chain-id=$CHAIN_ID --home "$VAL_HOME_DIR"
	done
	...
	for i in "${!MONIKERS[@]}"; do
		...
		export DAEMON_NAME=nemo-networkd
		...
		cosmovisor init /bin/nemo-networkd
...
# Modify any of the default configuration generated during `nemo-networkd init`. For seed nodes
