...
nemod tendermint gen-priv-key --home ./tmp-new-validator --mnemonic "$mnemonic"
address=$(nemod keys show val --address --keyring-backend=test --home ./tmp-new-validator)
node_key=$(jq -r ".priv_key.value" ./tmp-new-validator/config/node_key.json)
node_id=$(nemod tendermint show-node-id --home ./tmp-new-validator)
cons_address=$(nemod tendermint show-address --home ./tmp-new-validator)
