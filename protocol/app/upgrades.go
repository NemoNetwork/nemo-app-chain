...
func (app *App) setupUpgradeHandlers() {
	if app.UpgradeKeeper.HasHandler(v_1_0.UpgradeName) {
		panic(fmt.Sprintf("Cannot register duplicate upgrade handler '%s'", v_1_0.UpgradeName))
	}
	app.UpgradeKeeper.SetUpgradeHandler(
		v_1_0.UpgradeName,
		v_1_0.CreateUpgradeHandler(
			app.ModuleManager,
			app.configurator,
			app.VaultKeeper,
		),
