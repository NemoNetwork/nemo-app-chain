...
import (
	...
	"github.com/nemo-network/v4-chain/protocol/x/ratelimit"
	ratelimit_keeper "github.com/nemo-network/v4-chain/protocol/x/ratelimit/keeper"
...
func TestAppModuleBasic_RegisterGRPCGatewayRoutes(t *testing.T) {
	...
	registeredRoutes := []string{
		"/nemo-network/v4/ratelimit/list_limit_params",
		"/nemo-network/v4/ratelimit/capacity_by_denom",
	}
	...
	recorder := httptest.NewRecorder()
	req, err := http.NewRequest("GET", "/nemo-network/v4/ratelimit/foo/bar/baz", nil)
	require.NoError(t, err)
	router.ServeHTTP(recorder, req)
