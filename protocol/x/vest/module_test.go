...
import (
	...
	vest_keeper "github.com/nemo-network/v4-chain/protocol/x/vest/keeper"
...
func TestAppModuleBasic_RegisterGRPCGatewayRoutes(t *testing.T) {
	...
	req, err := http.NewRequest("GET", "/nemo-network/v4/vest/vest_entry", nil)
	require.NoError(t, err)
	...
	recorder = httptest.NewRecorder()
	req, err = http.NewRequest("GET", "/nemo-network/v4/vest/foo/bar/baz", nil)
	require.NoError(t, err)
	router.ServeHTTP(recorder, req)
