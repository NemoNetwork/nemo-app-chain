...
import (
	...
	bridge_servertypes "github.com/nemo-network/v4-chain/protocol/daemons/server/types/bridge"
	"github.com/nemo-network/v4-chain/protocol/mocks"
	"github.com/nemo-network/v4-chain/protocol/testutil/keeper"
	"github.com/nemo-network/v4-chain/protocol/x/bridge"
	bridge_keeper "github.com/nemo-network/v4-chain/protocol/x/bridge/keeper"
...
func TestAppModuleBasic_RegisterGRPCGatewayRoutes(t *testing.T) {
	...
	req, err := http.NewRequest("GET", "/nemo-network/v4/bridge/event_params", nil)
	require.NoError(t, err)
	...
	recorder = httptest.NewRecorder()
	req, err = http.NewRequest("GET", "/nemo-network/v4/bridge/propose_params", nil)
	require.NoError(t, err)
	router.ServeHTTP(recorder, req)
	require.Contains(t, recorder.Body.String(), "no RPC client is defined in offline mode")
	...
	recorder = httptest.NewRecorder()
	req, err = http.NewRequest("GET", "/nemo-network/v4/bridge/safety_params", nil)
	require.NoError(t, err)
	router.ServeHTTP(recorder, req)
	require.Contains(t, recorder.Body.String(), "no RPC client is defined in offline mode")
	...
	recorder = httptest.NewRecorder()
	req, err = http.NewRequest("GET", "/nemo-network/v4/bridge/acknowledged_event_info", nil)
	require.NoError(t, err)
	router.ServeHTTP(recorder, req)
	require.Contains(t, recorder.Body.String(), "no RPC client is defined in offline mode")
	...
	recorder = httptest.NewRecorder()
	req, err = http.NewRequest("GET", "/nemo-network/v4/bridge/recognized_event_info", nil)
	require.NoError(t, err)
	router.ServeHTTP(recorder, req)
	require.Contains(t, recorder.Body.String(), "no RPC client is defined in offline mode")
}
