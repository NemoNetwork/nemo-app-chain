...
import (
	...
	"github.com/nemo-network/v4-chain/protocol/lib/vault"
...
func (k Keeper) WithdrawFromMegavault(
	...
	// 0. Accrue fees, so that the exiting depositor pays their pro-rata share of
	//    fees earned up to this moment. See ADR-001 §6.
	...
	// 1a. Reject an operator withdrawal that would breach the operator share floor.
	if err := k.ValidateOperatorShareFloor(ctx, toSubaccount.Owner, sharesToWithdraw); err != nil {
		return nil, err
	}
