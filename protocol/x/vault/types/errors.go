...
var (
	...
	// Codes 32 and above have no upstream counterpart. Upstream's vault error
	// range ends at 31, so fork-local errors start here to keep future ports
	// mechanical.
	ErrNegativeDepositCap = errorsmod.Register(
		ModuleName,
		32,
		"Deposit cap must be non-negative",
	)
	ErrDepositCapExceeded = errorsmod.Register(
		ModuleName,
		33,
		"Deposit would push megavault equity above the deposit cap",
	)
	ErrInvalidFeePpm = errorsmod.Register(
		ModuleName,
		34,
		"Fee parts-per-million must not exceed 1_000_000",
	)
	ErrOperatorShareBelowMinimum = errorsmod.Register(
		ModuleName,
		35,
		"Withdrawal would take the operator's share below the configured minimum",
	)
	ErrNegativeHighWaterMark = errorsmod.Register(
		ModuleName,
		36,
		"High water mark must be non-negative",
	)
)
