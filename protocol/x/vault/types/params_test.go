...
func TestValidateMegavaultParams(t *testing.T) {
	tests := map[string]struct {
		params      types.MegavaultParams
		expectedErr error
	}{
		"Success - default params (uncapped)": {
			params: types.DefaultMegavaultParams(),
		},
		"Success - explicit zero cap means uncapped": {
			params: types.MegavaultParams{
				DepositCapQuoteQuantums: dtypes.NewInt(0),
			},
		},
		"Success - positive cap": {
			params: types.MegavaultParams{
				DepositCapQuoteQuantums: dtypes.NewInt(1_000_000_000),
			},
		},
		"Success - nil cap reads as uncapped": {
			// State written before `MegavaultParams` existed unmarshals to a nil
			// SerializableInt, which must validate rather than panic.
			params: types.MegavaultParams{},
		},
		"Failure - negative cap": {
			params: types.MegavaultParams{
				DepositCapQuoteQuantums: dtypes.NewInt(-1),
			},
			expectedErr: types.ErrNegativeDepositCap,
		},
	}
