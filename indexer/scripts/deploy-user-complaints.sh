...
echo "  curl -X POST http://localhost:3002/v4/userComplaints -H 'Content-Type: application/json' -d '{\"walletAddress\": \"nemo1abc\", \"message\": \"Unable to connect wallet\", \"email\": \"user@example.com\"}'"
echo ""
echo "Verify endpoint (without email):"
echo "  curl -X POST http://localhost:3002/v4/userComplaints -H 'Content-Type: application/json' -d '{\"walletAddress\": \"nemo1abc\", \"message\": \"Unable to connect wallet\"}'"
